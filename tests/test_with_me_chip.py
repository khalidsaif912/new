...
def test_with_me_page_exists():
    ...
    assert "transparenttextures.com/patterns/" in html
    assert "data-bg-texture-mount" in html
    assert "roster_banner_choice" not in html
    assert "/assets/banners/" not in html
    ...
    assert "shiftDayPin" not in html
    assert "paintShiftPin" not in src
    assert "height: calc(var(--day-active-h) + 12px)" in html
    assert "touch-action: none" in html
    assert "shiftDay" in src
    assert "--day-w: 40px" in html
    assert "--day-active-w: 74px" in html
    assert "--day-active-h: 88px" in html
    assert "margin-block: 8px 6px" in html
    assert "html.shift-strip-dragging" in html
    assert "overscroll-behavior: contain" in html
    assert "padding-block-start: 8px" in html
    ...
    assert "border-inline-start: 3px solid" in html
