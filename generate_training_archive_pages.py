...
:root {
  ...
  --border2:rgba(15,23,42,.09);
  --text:#152032;
  --muted:#64748b;
  --blue:#1d4ed8;
  --blue2:#2563eb;
  --teal:#0d9488;
  --shadow:0 14px 44px rgba(15,23,42,.07);
  --shadow-soft:0 6px 28px rgba(15,23,42,.06);
  --shadow-inset:inset 0 1px 0 rgba(255,255,255,.94);
  --header-grad:linear-gradient(118deg,#2563eb 0%,#1d4ed8 45%,#1e3a8a 100%);
  --accent-strip:linear-gradient(90deg,#0d9488 0%,#2dd4bf 38%,#2563eb 100%);
  --row-alt:rgba(248,250,252,.92);
  --row-fav:rgba(16,185,129,.11);
  --emp-no-bg:#eef2ff;
  --emp-no-c:#6366f1;
  --emp-code:#1e293b;
  --emp-name:#334155;
  --row-border:rgba(15,23,42,.05);
  --today-badge-bg:#dbeafe;
  --today-badge-c:#1d4ed8;
  --search-bg:#fff;
  --search-border:rgba(15,23,42,.1);
  --empty-bg:#f8fafc;
  --empty-border:rgba(15,23,42,.08);
  --footer-c:#94a3b8;
  --footer-strong:#64748b;
  --past-opacity:.44;
  --past-filter:saturate(.52) brightness(1.03);
}
*{box-sizing:border-box;margin:0;padding:0}
body{
  background:var(--bg);
  background-image:
    radial-gradient(ellipse 95% 55% at 50% -8%, rgba(37,99,235,.08), transparent 52%),
    linear-gradient(180deg,#eef2f7 0%,var(--bg) 40%,#f1f5f9 100%);
  color:var(--text);
...
.scheduleSection{margin-top:6px;position:relative;z-index:1}
.langBar{position:absolute;top:16px;right:18px;z-index:5;display:flex;justify-content:flex-end}
.langToggle{
  width:32px;height:32px;border:none;border-radius:999px;
  background:rgba(255,255,255,.2);color:#fff;
  font:800 11px/1 'IBM Plex Sans Arabic',sans-serif;cursor:pointer;
  display:grid;place-items:center;backdrop-filter:blur(8px);
  border:1px solid rgba(255,255,255,.24);
  transition:background .2s,transform .2s;
  box-shadow:0 4px 16px rgba(0,0,0,.12);
}
.langToggle:hover{background:rgba(255,255,255,.34);transform:scale(1.06)}
...
.header{
  ...
  box-shadow:0 20px 52px rgba(30,64,175,.26),0 6px 20px rgba(15,23,42,.08);
}
.header::before{
  content:'';position:absolute;inset:0;pointer-events:none;
  background:
    radial-gradient(circle at 88% 10%,rgba(255,255,255,.22) 0%,transparent 44%),
...
.hCircle2{width:168px;height:168px;right:88px;bottom:-72px}
.hCircle3{width:96px;height:96px;left:38%;top:-26px}
.headerText{position:relative;z-index:2;max-width:580px}
.header h1{
  margin:0;
  font:900 clamp(26px,5.2vw,36px)/1.06 'Sora',sans-serif;
  letter-spacing:-.045em;color:#fff;
  text-shadow:0 2px 24px rgba(0,0,0,.18);
}
.header p{
  margin:10px 0 0;font-size:13px;color:rgba(255,255,255,.88);
  font-weight:600;letter-spacing:.02em;
}
.dateTag{
  position:relative;z-index:2;display:inline-flex;align-items:center;gap:8px;margin-top:22px;
  padding:11px 40px 11px 16px;border-radius:999px;
  background:rgba(0,0,0,.14);
  border:1px solid rgba(255,255,255,.26);
  color:rgba(255,255,255,.96);font-size:13px;font-weight:700;
  backdrop-filter:blur(12px);
}
.dateTag::after{
  content:'▾';position:absolute;left:15px;top:50%;transform:translateY(-50%);
  color:rgba(255,255,255,.86);font-size:12px;font-weight:900;pointer-events:none;
}
.dateTagSelect{position:absolute;inset:0;opacity:0;cursor:pointer;border:none;appearance:none}
.topDock{
  display:flex;
...
.dockCard:active{transform:translateY(-1px)}
.dockAction{cursor:pointer;border:none;font-family:inherit;background:#fff}
.statChip{
  pointer-events:none;cursor:default;flex:0 0 auto;
}
.statChip:hover{transform:none;box-shadow:0 2px 8px rgba(15,23,42,.06);border-color:rgba(15,23,42,.1)}
.statsTicker{
  position:relative;width:100%;min-width:48px;height:42px;overflow:hidden;isolation:isolate;
}
.statsFace{
...
#otherPageBtn:hover .otherIcon{transform:translateY(-5px) scale(1.06)}
.rosterCard{display:block;text-decoration:none;color:inherit}
.rosterInner{position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:0;min-height:26px;width:100%}
.rosterIcon{
  width:34px;height:34px;display:block;flex-shrink:0;object-fit:contain;
  filter:drop-shadow(0 3px 8px rgba(15,23,42,.12));
  transform:translateY(-2px);
...
.rosterCard:hover .rosterIcon{transform:translateY(-5px) scale(1.06)}
...
.dockValue{font-size:22px;font-weight:900;line-height:1;height:26px;display:flex;align-items:center;justify-content:center;color:#1e40af;font-family:'Sora',sans-serif;letter-spacing:-.03em}
.searchGlyph{line-height:0;display:flex;align-items:center;justify-content:center;transition:transform .25s ease}
#searchToggle:hover .searchGlyph{transform:translateY(-3px) scale(1.05)}
...
.savedChip:hover{box-shadow:0 8px 20px rgba(37,99,235,.12)}
.savedIcon{
  width:auto;height:26px;flex:0 0 auto;border-radius:0;display:flex;align-items:center;justify-content:center;
  background:transparent;font-size:18px;border:none;
...
.myTrainingPanel{
  ...
  box-shadow:0 6px 22px rgba(13,148,136,.08);
}
.myTrainingPanel[hidden]{display:none!important}
.myTrainingHead{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:12px}
.myTrainingTitle{font:800 15px/1.25 'Sora',sans-serif;color:#0f766e}
.myTrainingSub{font-size:12px;color:var(--muted);margin-top:4px;line-height:1.35}
.myTrainingCountChip{
  flex:0 0 auto;pointer-events:none;cursor:default;
  background:#fff;border-color:rgba(13,148,136,.18);
}
.myTrainingCountChip:hover{transform:none;box-shadow:0 2px 8px rgba(15,23,42,.06)}
.myTrainingCountChip .dockValue{color:#0f766e}
.myTrainingCountChip .dockLabel{color:#64748b}
.myTrainingList{display:flex;flex-direction:column;gap:8px}
.myTrainingItem{
  display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:10px 12px;align-items:center;
  padding:10px 12px;border-radius:14px;background:#fff;border:1px solid rgba(15,23,42,.08);
  text-align:left;cursor:pointer;font-family:inherit;width:100%;
  transition:transform .2s,box-shadow .2s,border-color .2s;
}
.myTrainingItem:hover{transform:translateY(-1px);box-shadow:0 6px 16px rgba(15,23,42,.08);border-color:rgba(13,148,136,.22)}
.myTrainingItem.is-past{opacity:.62}
.myTrainingItem.is-today{border-color:rgba(37,99,235,.28);box-shadow:0 0 0 1px rgba(37,99,235,.12)}
.myTrainingDate{
  min-width:50px;text-align:center;padding:6px 8px;border-radius:12px;
  background:#eff6ff;color:#1d4ed8;font-weight:800;font-size:11px;line-height:1.15;
}
.myTrainingDate .dayNum{font-size:17px;display:block;line-height:1;font-family:'Sora',sans-serif}
.myTrainingMeta{min-width:0}
.myTrainingCourse{font-size:13px;font-weight:700;color:var(--text);line-height:1.3;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;font-family:'Sora',sans-serif}
.myTrainingCourseAr{
  margin-top:2px;font-size:11px;font-weight:700;line-height:1.35;color:var(--muted);
  font-family:'IBM Plex Sans Arabic','Segoe UI',Tahoma,sans-serif;
  display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;
}
.myTrainingVenue{font-size:11px;color:var(--muted);margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.myTrainingTime{font-size:11px;font-weight:700;color:#0f766e;white-space:nowrap}
.myTrainingEmpty{
  padding:16px 12px;text-align:center;font-size:13px;color:var(--muted);
  border:1px dashed rgba(15,23,42,.12);border-radius:14px;background:#fff;line-height:1.45;
}
.filters{
  display:none;margin-top:4px;margin-bottom:4px;padding:16px 16px 17px;border-radius:22px;
  background:linear-gradient(165deg,#fafbfc 0%,#f1f5f9 100%);
  border:1px solid var(--border2);
  box-shadow:var(--shadow-inset),0 4px 20px rgba(15,23,42,.04);
  position:relative;z-index:1;
}
.filters.active{display:block}
.searchRow{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
.searchRow input{
  flex:1;min-width:0;padding:13px 16px;border-radius:18px;
...
.searchRow input::placeholder{color:var(--muted);opacity:.65}
.searchBtn,.clearBtn{border:none;border-radius:16px;padding:12px 16px;font-size:12px;font-weight:800;cursor:pointer;font-family:inherit}
.searchBtn{
  background:linear-gradient(135deg,var(--blue2),var(--blue));color:#fff;min-width:88px;
  box-shadow:0 8px 24px rgba(29,78,216,.3);
}
.clearBtn{background:#e2e8f0;color:#334155;font-weight:700}
.cards{margin-top:18px;display:flex;flex-direction:column;gap:16px}
.pastLabel{
  display:flex;align-items:center;gap:14px;margin:22px 0 10px;
  color:var(--muted);font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.14em;
}
.pastLabel::before,.pastLabel::after{content:'';flex:1;height:1px;background:linear-gradient(90deg,transparent,rgba(148,163,184,.5) 20%,rgba(148,163,184,.5) 80%,transparent)}
.courseCard{
  background:var(--panel);border:1px solid rgba(15,23,42,.06);border-radius:26px;overflow:hidden;
  box-shadow:0 8px 32px rgba(15,23,42,.06),var(--shadow-inset);
  transition:border-color .3s,box-shadow .3s,opacity .25s,filter .25s,transform .22s;
}
.courseCard[open]{transform:translateY(-2px);box-shadow:0 16px 44px rgba(15,23,42,.1)}
.courseCard.is-today{box-shadow:0 0 0 2px rgba(37,99,235,.2),0 16px 44px rgba(37,99,235,.12)}
.courseCard.is-saved{box-shadow:0 0 0 2px rgba(13,148,136,.22),0 12px 36px rgba(13,148,136,.08)}
.courseCard.is-past{opacity:var(--past-opacity);filter:var(--past-filter)}
.courseCard.pastelT0 .courseHead{
  --accent:#f59e0b;--surface:#fffbeb;--surface2:#fef3c7;--pill:rgba(245,158,11,.35);--text-on-acc:#92400e;
}
.courseCard.pastelT0 .courseBody{background:linear-gradient(180deg,#fffdfb,#fffaf0)}
.courseCard.pastelT0 .rowsWrap{border-color:rgba(245,158,11,.14)}
.courseCard.pastelT1 .courseHead{
  --accent:#fb7185;--surface:#fff1f2;--surface2:#ffe4e6;--pill:rgba(251,113,133,.3);--text-on-acc:#9f1239;
}
.courseCard.pastelT1 .courseBody{background:linear-gradient(180deg,#fffafa,#fff5f7)}
.courseCard.pastelT1 .rowsWrap{border-color:rgba(251,113,133,.14)}
.courseCard.pastelT2 .courseHead{
  --accent:#a78bfa;--surface:#f5f3ff;--surface2:#ede9fe;--pill:rgba(167,139,250,.35);--text-on-acc:#5b21b6;
}
.courseCard.pastelT2 .courseBody{background:linear-gradient(180deg,#fafaff,#f3f0ff)}
.courseCard.pastelT2 .rowsWrap{border-color:rgba(167,139,250,.15)}
.courseCard.pastelT3 .courseHead{
  --accent:#2dd4bf;--surface:#ecfdf5;--surface2:#d1fae5;--pill:rgba(45,212,191,.35);--text-on-acc:#0f766e;
}
.courseCard.pastelT3 .courseBody{background:linear-gradient(180deg,#f6fffb,#ecfdf5)}
.courseCard.pastelT3 .rowsWrap{border-color:rgba(45,212,191,.14)}
.courseCard.pastelT4 .courseHead{
  --accent:#60a5fa;--surface:#eff6ff;--surface2:#dbeafe;--pill:rgba(96,165,250,.4);--text-on-acc:#1e40af;
}
.courseCard.pastelT4 .courseBody{background:linear-gradient(180deg,#fafcff,#f0f7ff)}
.courseCard.pastelT4 .rowsWrap{border-color:rgba(96,165,250,.15)}
.courseHead{
  position:relative;display:grid;grid-template-columns:48px minmax(0,1fr) auto;gap:14px;align-items:start;
  padding:18px 18px 16px;cursor:pointer;list-style:none;
  background:
    radial-gradient(ellipse 80% 60% at 100% 0%,rgba(255,255,255,.75),transparent 50%),
    linear-gradient(175deg,var(--surface,#f8fafc),var(--surface2,#e2e8f0));
  border-inline-start:5px solid var(--accent,#2563eb);
  box-shadow:0 4px 20px rgba(15,23,42,.05);
  transition:background .3s,box-shadow .3s;
}
.courseCard[open] .courseHead{box-shadow:0 8px 28px rgba(15,23,42,.08)}
.courseHead::-webkit-details-marker{display:none}
.headGlow{
  position:absolute;inset:0;pointer-events:none;
  background:linear-gradient(105deg,rgba(255,255,255,.45),transparent 45%);
}
.courseIcon{
...
.badge{
  ...
  padding:7px 12px;border-radius:999px;font-size:10px;font-weight:800;letter-spacing:.03em;
...
.empRow{
  display:grid;grid-template-columns:28px minmax(76px,100px) 1fr;gap:12px;align-items:center;
  padding:12px 14px;background:rgba(255,255,255,.75);border-top:1px solid var(--row-border);
}
...
.empNo{
  ...
  min-width:24px;width:24px;height:24px;border-radius:999px;
  background:var(--emp-no-bg);color:var(--emp-no-c);
  font-size:9px;font-weight:800;
  box-shadow:0 2px 6px rgba(99,102,241,.12);
}
.empCode{font-size:12px;font-weight:700;color:var(--emp-code);letter-spacing:.03em;font-variant-numeric:tabular-nums}
.empName{min-width:0;font-size:13px;font-weight:600;line-height:1.4;color:var(--emp-name);display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.favBadge{padding:3px 9px;border-radius:999px;font-size:9px;font-weight:800;background:rgba(13,148,136,.15);color:#0f766e}
.emptyState{
  display:none;margin-top:16px;padding:28px;text-align:center;color:var(--muted);
  border-radius:22px;background:var(--empty-bg);border:2px dashed var(--empty-border);
  font-size:14px;font-weight:500;
}
.footer{
  margin-top:32px;padding:22px 16px 12px;text-align:center;color:var(--footer-c);
  font-size:12px;line-height:1.55;
}
.footer strong{color:var(--footer-strong);font-weight:600}
.footerShare{margin-top:18px;display:flex;justify-content:center;padding:0 12px 4px}
.gamesPanel{
  margin:30px auto 0;max-width:560px;padding:14px 14px 13px;text-align:center;
  background:#fff;border:1px solid #e2e8f0;border-radius:20px;
  box-shadow:0 4px 14px rgba(15,23,42,.06);
}
.gamesPanelTitle{
  display:flex;align-items:center;justify-content:center;gap:7px;
  font-size:13px;font-weight:800;color:#334155;margin-bottom:11px;
  font-family:'Sora','IBM Plex Sans Arabic',sans-serif;
}
.gamesGrid{display:flex;flex-wrap:wrap;justify-content:center;gap:8px}
.gameChip{
  display:inline-flex;align-items:center;gap:7px;min-height:38px;padding:8px 14px;
  border-radius:999px;font-size:12px;font-weight:700;color:#0f172a;text-decoration:none;
  border:1.5px solid #e2e8f0;background:#f8fafc;line-height:1.2;
  touch-action:manipulation;-webkit-tap-highlight-color:transparent;
  transition:transform .15s ease,box-shadow .15s ease,filter .15s ease;
}
.gameChip:hover{transform:translateY(-1px);box-shadow:0 5px 12px rgba(15,23,42,.1);filter:saturate(1.1)}
.gameChip:active{transform:translateY(0)}
.gameChipIcon{font-size:16px;line-height:1}
.gameChip--pairs{background:#eef2ff;border-color:#c7d2fe}
.gameChip--similarity{background:#fdf4ff;border-color:#f5d0fe}
.gameChip--quiz{background:#fff7ed;border-color:#fed7aa}
.gameChip--air{background:#eff6ff;border-color:#bfdbfe}
.gameChip--cargo{background:#ecfdf5;border-color:#a7f3d0}
html[lang="ar"] .gamesGrid,body.ar .gamesGrid{direction:rtl}
.trainingShareBtn{
  display:inline-flex;align-items:center;justify-content:center;gap:8px;
  min-height:44px;padding:10px 18px;border-radius:999px;border:1.5px solid #86efac;
  background:#ecfdf5;color:#166534;font-family:inherit;font-size:13px;font-weight:700;
  line-height:1.2;cursor:pointer;touch-action:manipulation;-webkit-tap-highlight-color:transparent;
  box-shadow:0 2px 8px rgba(22,101,52,.08);transition:transform .15s ease,box-shadow .15s ease,background .15s ease;
  max-width:min(100%,320px);width:100%;
}
.trainingShareBtn:hover{transform:translateY(-1px);box-shadow:0 6px 16px rgba(22,101,52,.14);background:#d1fae5}
.trainingShareBtn:active{transform:translateY(0)}
.trainingShareIcon{flex-shrink:0;display:inline-flex;align-items:center;justify-content:center;width:18px;height:18px;line-height:0}
.trainingShareIcon svg{display:block;width:18px;height:18px}
.siteShareSheet{
...
.siteShareTitle{font-size:17px;font-weight:800;color:#0f172a;margin:0 0 4px;font-family:'Sora',sans-serif}
.siteShareHint{font-size:12px;color:#64748b;margin:0 0 14px;line-height:1.4}
.siteShareQr{
  display:flex;align-items:center;justify-content:center;
  min-height:220px;margin:0 auto 12px;background:#f8fafc;border-radius:14px;border:1px solid #e2e8f0;padding:10px;
}
.siteShareUrl{
...
.siteShareActions .trainingShareBtn{max-width:none;width:auto;min-height:40px;padding:9px 12px;font-size:12px;border-radius:999px}
.siteShareActions .trainingShareBtn--wide{grid-column:1 / -1}
.siteShareActions .trainingShareBtn--muted{background:#f1f5f9;border-color:#cbd5e1;color:#475569;box-shadow:none}
.siteShareActions .trainingShareBtn--wa{background:#ecfdf5;border-color:#86efac;color:#166534}
.siteShareActions .trainingShareBtn--copy{background:#fffbeb;border-color:#fcd34d;color:#92400e}
.siteShareCloseWrap{margin-top:4px}
.siteShareCloseWrap .trainingShareBtn{max-width:none}
.headerRow{margin-top:18px;position:relative;z-index:2}
.archiveBtn{
  display:inline-flex;align-items:center;gap:8px;padding:11px 18px;border-radius:999px;
  background:rgba(255,255,255,.24);border:1px solid rgba(255,255,255,.3);
  color:#fff;font-size:13px;font-weight:800;text-decoration:none;
  backdrop-filter:blur(10px);transition:transform .2s,background .2s,box-shadow .2s;
  box-shadow:0 6px 20px rgba(0,0,0,.12);
}
.archiveBtn:hover{background:rgba(255,255,255,.34);transform:translateY(-2px);box-shadow:0 10px 26px rgba(0,0,0,.14)}
.archiveGrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin-top:18px}
.archiveCard{
  display:block;text-decoration:none;color:var(--text);background:var(--panel);
  border:1px solid var(--border2);border-radius:26px;padding:20px;
  box-shadow:var(--shadow-soft);transition:transform .22s,box-shadow .22s;
}
.archiveCard:hover{transform:translateY(-3px);box-shadow:0 16px 40px rgba(15,23,42,.1)}
.archiveMonth{font-size:19px;font-weight:900;font-family:'Sora',sans-serif}
.archiveMeta{margin-top:10px;color:var(--muted);font-size:13px;line-height:1.45}
.archiveLatest{display:inline-flex;margin-top:12px;padding:6px 11px;border-radius:999px;background:#e0e7ff;color:#3730a3;font-size:10px;font-weight:800}
.staffModal{
  position:fixed;inset:0;display:none;align-items:center;justify-content:center;
  background:rgba(15,23,42,.4);backdrop-filter:blur(8px);z-index:50;padding:20px;
}
.staffModal.open{display:flex}
.staffModalCard{
  width:min(100%,380px);background:#fff;border:1px solid rgba(15,23,42,.08);
  border-radius:28px;box-shadow:0 28px 80px rgba(15,23,42,.25);padding:22px 22px 18px;
}
.staffModalTop{display:flex;align-items:center;justify-content:space-between;gap:12px}
.staffModalTitle{font:800 17px/1.15 'Sora',sans-serif;color:var(--text)}
.staffModalSub{margin-top:8px;font-size:13px;color:var(--muted)}
.staffClose{
  width:38px;height:38px;border:none;border-radius:14px;background:#eff6ff;color:var(--blue);
  font:900 18px/1 'Sora',sans-serif;cursor:pointer;box-shadow:0 2px 8px rgba(37,99,235,.12);
}
.staffField{
  margin-top:16px;width:100%;height:50px;border-radius:18px;border:1px solid rgba(15,23,42,.1);
  background:#f8fafc;padding:0 16px;color:var(--text);
  font:600 14px/1 'Plus Jakarta Sans','IBM Plex Sans Arabic',Tahoma,sans-serif;outline:none;
}
.staffField:focus{border-color:rgba(37,99,235,.45);box-shadow:0 0 0 4px rgba(37,99,235,.1)}
.staffModalActions{display:flex;gap:10px;margin-top:16px}
.staffBtn{
  flex:1;height:46px;border:none;border-radius:16px;cursor:pointer;
  font:800 13px/1 'Plus Jakarta Sans','IBM Plex Sans Arabic',Tahoma,sans-serif;
}
.staffBtnPrimary{background:linear-gradient(135deg,var(--blue2),var(--blue));color:#fff;box-shadow:0 8px 22px rgba(29,78,216,.28)}
.staffBtnGhost{background:#eff6ff;color:var(--blue);font-weight:700}
@media(max-width:720px){
  .wrap{padding:14px 12px 36px}
  .mainPanel{padding:22px 14px 24px;border-radius:26px}
  .mainPanel::before{border-radius:26px 26px 0 0}
  .header{padding:24px 18px 22px;border-radius:28px}
  .header::after{font-size:56px;left:4%;bottom:8%}
  .topDock{gap:6px;margin-top:14px}
  .dockCard{padding:8px 8px;min-width:60px;border-radius:14px}
  ...
  .courseHead{grid-template-columns:42px minmax(0,1fr) auto;gap:10px;padding:14px 14px}
  .courseIcon{width:42px;height:42px;font-size:19px;border-radius:16px}
  .courseTitle{font-size:15px}
  .courseBody{padding:14px 14px 16px}
  .filters{padding:14px;border-radius:18px}
  .searchRow input{padding:12px 14px;font-size:13px}
  .empRow{grid-template-columns:26px minmax(68px,88px) 1fr;gap:10px;padding:10px 12px}
}
@media(max-width:420px){
  .wrap{padding:12px 10px 30px}
  .header{padding:22px 14px 18px;border-radius:24px}
  .topDock{gap:6px}
  .dockCard{padding:8px 6px;min-width:56px}
...
SHARE_ICON = (
    ...
    '<path d="M4 12v7a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-7"/><path d="M12 3v12M8 7l4-4 4 4"/></svg>'
)
...
<div id="siteShareSheet" class="siteShareSheet" aria-hidden="true">
  <div class="siteShareCard" role="dialog" aria-labelledby="siteShareTitle">
    <h2 class="siteShareTitle" id="siteShareTitle">Share training page</h2>
    <p class="siteShareHint" id="siteShareHint">Scan the QR code or copy the link</p>
    <div class="siteShareQr" id="siteShareQr"></div>
    <p class="siteShareUrl" id="siteShareUrl"></p>
    <div class="siteShareActions">
      <button type="button" class="trainingShareBtn trainingShareBtn--wa" id="siteShareNativeBtn">
        <span class="trainingShareIcon">{SHARE_ICON}</span>
        <span class="roster-cta-label">Share</span>
      </button>
      <button type="button" class="trainingShareBtn trainingShareBtn--wa" id="siteShareWhatsAppBtn">
        <span class="trainingShareIcon"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="9" fill="#22c55e"/><path d="M8.5 9.5c.4 2.2 2.4 4.2 4.8 4.8l1-2.2c.1-.2.3-.3.5-.2l1.8.8c.2.1.4 0 .5-.2.4-.9.9-1.7 1.5-2.4.1-.2 0-.5-.2-.6l-1.6-.9c-.2-.1-.5 0-.6.2-.3.6-.7 1.1-1.1 1.6-.1.2-.4.2-.6.1l-1.4-.7c-.2-.1-.4-.1-.5.1z" fill="#fff"/></svg></span>
        <span class="roster-cta-label">WhatsApp</span>
      </button>
      <button type="button" class="trainingShareBtn trainingShareBtn--copy trainingShareBtn--wide" id="siteShareCopyBtn">
        <span class="trainingShareIcon"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7.07 0l1.41-1.41a5 5 0 0 0-7.07-7.07L10 5"/><path d="M14 11a5 5 0 0 0-7.07 0L5.52 12.41a5 5 0 0 0 7.07 7.07L14 19"/></svg></span>
        <span class="roster-cta-label">Copy link</span>
      </button>
    </div>
    <div class="siteShareCloseWrap">
      <button type="button" class="trainingShareBtn trainingShareBtn--muted trainingShareBtn--wide" id="siteShareCloseBtn">
...
<div class="footerShare">
  <button type="button" class="trainingShareBtn" id="shareSiteBtn">
    <span class="trainingShareIcon">{SHARE_ICON}</span>
    <span class="roster-cta-label">Share page</span>
  </button>
</div>
'''
...
<div class="gamesPanel">
  <div class="gamesPanelTitle"><span aria-hidden="true">🎮</span><span data-tr-en="Break-time games" data-tr-ar="ألعاب الاستراحة">Break-time games</span></div>
  <div class="gamesGrid">
    <a class="gameChip gameChip--pairs" href="https://dgr-exp.netlify.app/m1/pairs.html" target="_blank" rel="noopener">
      <span class="gameChipIcon" aria-hidden="true">🧩</span><span data-tr-en="Pairs" data-tr-ar="قرائن الصور">Pairs</span>
    </a>
    <a class="gameChip gameChip--similarity" href="https://dgr-exp.netlify.app/g2/similarity.html" target="_blank" rel="noopener">
      <span class="gameChipIcon" aria-hidden="true">🧠</span><span data-tr-en="Similarity" data-tr-ar="التشابه">Similarity</span>
    </a>
    <a class="gameChip gameChip--quiz" href="https://dgr-exp.netlify.app/Q/Q.HTML" target="_blank" rel="noopener">
      <span class="gameChipIcon" aria-hidden="true">🎯</span><span data-tr-en="The Question" data-tr-ar="السؤال">The Question</span>
    </a>
    <a class="gameChip gameChip--air" href="https://dgr-exp.netlify.app/g1/airlogo.html" target="_blank" rel="noopener">
      <span class="gameChipIcon" aria-hidden="true">✈️</span><span data-tr-en="Air Logo" data-tr-ar="Air Logo">Air Logo</span>
    </a>
    <a class="gameChip gameChip--cargo" href="https://dgr-exp.netlify.app/g3/cargo-dash.html" target="_blank" rel="noopener">
      <span class="gameChipIcon" aria-hidden="true">📦</span><span data-tr-en="Cargo Dash" data-tr-ar="شريط التفتيش">Cargo Dash</span>
    </a>
  </div>
</div>
'''
...
def share_script_src(in_archive: bool) -> str:
    return f"../../site-share.js?v={SHARE_VER}" if in_archive else f"../site-share.js?v={SHARE_VER}"


PAGE_JS = r"""
(function(){
  const SAVED_QUERY_KEY = 'trainingSavedQuery';
  const ROSTER_EMP_ID_KEYS = ['exportSavedEmpId', 'savedEmpId', 'importSavedEmpId'];
  const ROSTER_EMP_NAME_KEYS = ['exportSavedEmpName', 'savedEmpName', 'importSavedEmpName'];
  ...
  const myTrainingPanel = document.getElementById('myTrainingPanel');
  const myTrainingTitle = document.getElementById('myTrainingTitle');
  const myTrainingSub = document.getElementById('myTrainingSub');
  const myTrainingCountVal = document.getElementById('myTrainingCountVal');
  ...
  function getRosterSavedEmpName(){
    for(const key of ROSTER_EMP_NAME_KEYS){
      const value = (localStorage.getItem(key) || '').trim();
      if(value) return value;
    }
    return '';
  }
  function rememberRosterName(empId, fullName){
    const name = (fullName || '').trim();
    const id = (empId || '').trim();
    if(!name || !id) return;
    localStorage.setItem('exportSavedEmpName', name);
    localStorage.setItem('savedEmpName', name);
    if(localStorage.getItem('importSavedEmpId') === id){
      localStorage.setItem('importSavedEmpName', name);
    }
  }
  function getSavedQuery(){
    const rosterId = getRosterSavedEmpId();
    if(rosterId) return rosterId;
    const rosterName = getRosterSavedEmpName();
    if(rosterName) return rosterName;
    return (localStorage.getItem(SAVED_QUERY_KEY) || '').trim();
  }
  function persistSavedQuery(value){
    const trimmed = (value || '').trim();
    if(!trimmed){
      localStorage.removeItem(SAVED_QUERY_KEY);
      ROSTER_EMP_ID_KEYS.forEach(key => localStorage.removeItem(key));
      ROSTER_EMP_NAME_KEYS.forEach(key => localStorage.removeItem(key));
      return;
    }
    localStorage.setItem(SAVED_QUERY_KEY, trimmed);
    if(/^\d+$/.test(trimmed)){
      const prevId = (localStorage.getItem('exportSavedEmpId') || '').trim();
      localStorage.setItem('exportSavedEmpId', trimmed);
      localStorage.removeItem('savedEmpId');
      if(prevId && prevId !== trimmed){
        localStorage.removeItem('exportSavedEmpName');
        localStorage.removeItem('savedEmpName');
      }
    }
  }
  function getSchedulesBase(){
    if(window.location.pathname.includes('/new/docs/')) return '/new/docs/schedules/';
    if(window.location.pathname.includes('/roster-site/')) return '/roster-site/schedules/';
    return '/schedules/';
  }
  function getImportSchedulesBase(){
    const base = getSchedulesBase();
    if(base.includes('/import/schedules/')) return base;
    return base.replace(/\/schedules\/$/, '/import/schedules/');
  }
  function fetchRosterDisplayName(empId, done){
    if(!empId || !/^\d+$/.test(empId)){ done(''); return; }
    const urls = [
      getSchedulesBase() + encodeURIComponent(empId) + '.json',
      getImportSchedulesBase() + encodeURIComponent(empId) + '.json'
    ];
    function tryNext(i){
      ...
      fetch(urls[i])
        ...
        .then(data => {
          if(data && data.name) done(data.name);
          else tryNext(i + 1);
        })
        .catch(() => tryNext(i + 1));
    }
  ...
  function rowMatchesQuery(row, q){
    ...
    return code === q || name.includes(q) || (code && q.includes(code));
  }
  function sessionFromRow(row){
    const card = row.closest('.courseCard');
    if(!card) return null;
    return {
      row,
      card,
      ...
      dateLabel: card.querySelector('.dateBadge')?.textContent?.trim() || '',
      isPast: card.classList.contains('is-past'),
      isToday: card.classList.contains('is-today') || !!card.querySelector('.todayBadge')
    };
  }
  function findAllSavedSessions(raw){
    const q = norm(raw);
    if(!q) return [];
    const seen = new Set();
    const sessions = [];
    for(const row of allRows()){
      ...
      if(!session || !session.card) continue;
      const key = (session.dateIso || '') + '|' + session.title;
  ...
  function scrollToSession(session){
    ...
    session.card.open = true;
    session.card.scrollIntoView({behavior:'smooth', block:'center'});
    session.row?.classList.add('favorite');
  }
  function renderMyTrainingPanel(raw, sessions, displayName, staffCode){
    if(!myTrainingPanel || !myTrainingList) return;
    if(!raw){
      myTrainingPanel.hidden = true;
      myTrainingList.innerHTML = '';
      return;
    }
    myTrainingPanel.hidden = false;
    const ar = document.documentElement.lang === 'ar' || document.body.classList.contains('ar');
    const who = displayName || raw;
    const codeText = staffCode ? (' · ' + staffCode) : '';
    if(myTrainingTitle) myTrainingTitle.textContent = ar ? 'مواعيد التدريب' : 'My training dates';
    if(myTrainingSub) myTrainingSub.textContent = ar
      ? ('لـ ' + who + codeText)
      : ('For ' + who + codeText);
    if(myTrainingCountVal) myTrainingCountVal.textContent = String(sessions.length);
    if(myTrainingCountLbl) myTrainingCountLbl.textContent = ar
      ? (sessions.length === 1 ? 'دورة' : 'دورات')
      : (sessions.length === 1 ? 'Session' : 'Sessions');
    if(!sessions.length){
      myTrainingList.innerHTML = '<div class="myTrainingEmpty">' + (ar
        ? 'لا توجد دورات مسجّلة لهذا الرقم في هذا الشهر.'
        : 'No courses found for this staff number in this month.') + '</div>';
      return;
    }
    myTrainingList.innerHTML = sessions.map((session, idx) => {
      ...
      const titleEn = session.title || '';
      const titleAr = session.titleAr || '';
      const venue = translateVenueValue(session.venue || '', ar) || (d.mon + ' ' + d.dayNum);
      const timeTxt = formatSessionTime(session.time || '', ar);
      const safeTitleEn = String(titleEn).replace(/</g, '&lt;').replace(/>/g, '&gt;');
      const safeTitleAr = String(titleAr).replace(/</g, '&lt;').replace(/>/g, '&gt;');
      const safeVenue = String(venue || '').replace(/</g, '&lt;').replace(/>/g, '&gt;');
      const safeTime = String(timeTxt || '').replace(/</g, '&lt;').replace(/>/g, '&gt;');
      const arLine = (titleAr && titleAr !== titleEn)
        ? ('<div class="myTrainingCourseAr">' + safeTitleAr + '</div>')
        : '';
      return (
        '<button type="button" class="' + cls + '" data-session-idx="' + idx + '">' +
          ...
          '<div class="myTrainingMeta">' +
            '<div class="myTrainingCourse">' + safeTitleEn + '</div>' +
            arLine +
    ...
    myTrainingList.querySelectorAll('[data-session-idx]').forEach(btn => {
      btn.addEventListener('click', () => {
        const idx = Number(btn.getAttribute('data-session-idx'));
        scrollToSession(sessions[idx]);
      });
    });
  }
  function refreshSaved(){
    ...
    const raw = getSavedQuery();
    const sessions = findAllSavedSessions(raw);
    sessions.forEach(session => {
      session.row.classList.add('favorite');
      const nameCell = session.row.querySelector('.empName');
      if(nameCell && !nameCell.querySelector('.favBadge')){
        const b = document.createElement('span');
        b.className = 'favBadge';
        b.textContent = document.documentElement.lang === 'ar' ? 'محفوظ' : 'Saved';
        nameCell.appendChild(b);
      }
      session.card.classList.add('is-saved');
    });
    ...
    const rosterName = getRosterSavedEmpName();
    if(primary){
      displayName = firstName(primary.name) || primary.name;
      staffCode = primary.code;
      ...
      if(rosterName){
        displayName = firstName(rosterName) || rosterName;
      } else if(staffCode && /^\d+$/.test(raw)){
        displayName = ar ? 'جاري التحميل…' : 'Loading…';
      } else {
        displayName = firstName(raw) || raw;
      }
      savedChip.classList.remove('savedEmpty');
      savedName.textContent = displayName;
      if(staffCode){
        fetchRosterDisplayName(staffCode, (fullName) => {
          if(!fullName) return;
          if(getSavedQuery() !== staffCode && getRosterSavedEmpId() !== staffCode) return;
          rememberRosterName(staffCode, fullName);
          const label = firstName(fullName) || fullName;
          savedName.textContent = label;
          renderMyTrainingPanel(staffCode, findAllSavedSessions(staffCode), label, staffCode);
        });
      }
    } else {
  ...
  function openStaffModal(){
    ...
    const current = getSavedQuery();
    const rosterName = getRosterSavedEmpName();
    if(staffInput) staffInput.value = current;
    const sub = staffModal.querySelector('.staffModalSub');
    if(sub){
      const ar = document.documentElement.lang === 'ar' || document.body.classList.contains('ar');
      if(current && rosterName){
        sub.textContent = ar
          ? ('محفوظ من الروستر: ' + rosterName)
          : ('Saved from roster: ' + rosterName);
      } else if(current){
        sub.textContent = ar
          ? 'تم جلب بياناتك من الروستر تلقائياً. يمكنك التعديل أو المسح.'
          : 'Loaded automatically from your roster. You can edit or clear.';
      } else {
        sub.textContent = ar
          ? 'يستخدم رقمك المحفوظ في الروستر إن وجد، أو أدخل الرقم / الاسم'
          : 'Uses your roster saved ID if available, or enter staff number / name';
      }
    }
  ...
  staffSave?.addEventListener('click', () => {
    let value = (staffInput?.value || '').trim();
    const idMatch = value.match(/^(\d{3,})/);
    if(idMatch) value = idMatch[1];
    persistSavedQuery(value);
    refreshSaved();
    applyFilters();
    closeStaffModal();
    if(value && myTrainingPanel && !myTrainingPanel.hidden){
      myTrainingPanel.scrollIntoView({behavior:'smooth', block:'nearest'});
    }
  });
  ...
  window.addEventListener('storage', (e) => {
    if(!e.key) return;
    if([SAVED_QUERY_KEY, ...ROSTER_EMP_ID_KEYS, ...ROSTER_EMP_NAME_KEYS].includes(e.key)){
      refreshSaved();
      applyFilters();
    }
    if(e.key === 'rosterLang'){
      applyTrainingLang(e.newValue === 'ar' ? 'ar' : 'en');
    }
  });
  ...
  const TRAIN_UI = {
    en: {
      ...
      time: 'Time',
      today: 'Today',
      past: 'Previous sessions',
      empty: 'No matching courses found.',
      saveEmp: 'Save employee',
      saveEmpSub: 'Uses your roster saved ID if available, or enter staff number / name',
      saveEmpPh: 'e.g. 8715 or Khalid',
      save: 'Save',
    ...
    ar: {
      ...
      saveEmp: 'حفظ الموظف',
      saveEmpSub: 'يستخدم رقمك المحفوظ في الروستر إن وجد، أو أدخل الرقم / الاسم',
      saveEmpPh: 'مثال: 8715 أو خالد',
      save: 'حفظ',
  ...
  function trainingLang(){
    const stored = localStorage.getItem('rosterLang');
    if(stored === 'ar' || stored === 'en') return stored;
    return (document.documentElement.lang === 'ar') ? 'ar' : 'en';
  }

  let NAME_AR_MAP = null;
  let NAME_AR_LOADING = false;
  const NAME_AR_WAITERS = [];
  ...
  function arabicForEnglishName(en){
    if(!NAME_AR_MAP || !en) return '';
    const key = normalizeNameKey(en);
    if(NAME_AR_MAP[key]) return NAME_AR_MAP[key];
    const stripped = key.replace(/^(MR\.|MRS\.|MISS\.|MS\.)\s*/, '').trim();
    const prefixes = ['', 'MR. ', 'MISS. ', 'MISS.', 'MRS. ', 'MS. ', 'MR.', 'MRS.', 'MS.'];
  ...
  function translateVenueValue(enVal, isAr){
    const raw = String(enVal || '').trim();
    if(!isAr) return raw;
    if(/cargo training room/i.test(raw)) return 'قاعة تدريب الشحن';
    return raw.replace(/\bto\b/gi, 'إلى');
  }

  function applyTrainingLang(lang){
    const L = (lang === 'ar') ? 'ar' : 'en';
    const t = TRAIN_UI[L];
    const isAr = L === 'ar';
    localStorage.setItem('rosterLang', L);
    document.documentElement.lang = L;
    document.documentElement.setAttribute('dir', isAr ? 'rtl' : 'ltr');
    document.body.classList.toggle('ar', isAr);

    const titleText = document.querySelector('#pageTitle .pageTitleText') || document.getElementById('pageTitle');
    if(titleText){
      if(titleText.classList && titleText.classList.contains('pageTitleText')) titleText.textContent = t.pageTitle;
      else {
        const span = titleText.querySelector('.pageTitleText');
        if(span) span.textContent = t.pageTitle;
      }
    }
    const sub = document.getElementById('pageSubtitle');
    if(sub) sub.textContent = t.pageSubtitle;

    const dateTag = document.querySelector('.dateTag');
    if(dateTag){
      dateTag.childNodes.forEach(node => {
        if(node.nodeType === 3){
          const raw = node.textContent || '';
          const trimmed = raw.trim();
          if(!trimmed) return;
          const range = trimmed.replace(/^(During|خلال)\s*/i, '').trim();
          node.textContent = ' ' + t.during + ' ' + range + ' ';
        }
      });
    }
    ...
    if(pastSep) pastSep.textContent = t.past;
    if(monthPicker) monthPicker.setAttribute('aria-label', t.chooseMonth);

    const statLabels = [t.sessions, t.types, t.employees, t.venues];
    document.querySelectorAll('.statsFace .dockLabel').forEach((el, i) => {
      if(statLabels[i]) el.textContent = statLabels[i];
    });

    document.querySelectorAll('.miniMeta').forEach(meta => {
      const strong = meta.querySelector('strong');
      if(!strong) return;
      let rest = '';
      meta.childNodes.forEach(n => {
        if(n !== strong && n.nodeType === 3) rest += n.textContent;
      });
      rest = rest.trim();
      if(!meta.dataset.enRest) meta.dataset.enRest = rest;
      const enRest = meta.dataset.enRest;
      const labelKey = (strong.textContent || '').trim().toLowerCase();
      const isVenue = labelKey === 'venue' || labelKey === 'المكان' || meta.dataset.kind === 'venue';
      const isTime = labelKey === 'time' || labelKey === 'الوقت' || meta.dataset.kind === 'time';
      if(isVenue) meta.dataset.kind = 'venue';
      if(isTime) meta.dataset.kind = 'time';
      strong.textContent = (meta.dataset.kind === 'venue') ? t.venue : t.time;
      let textNode = null;
      meta.childNodes.forEach(n => {
        if(n !== strong && n.nodeType === 3){
          if(!textNode) textNode = n;
          else n.textContent = '';
        }
      });
      const shown = ' ' + translateVenueValue(enRest, isAr);
      if(textNode) textNode.textContent = shown;
      else meta.appendChild(document.createTextNode(shown));
    });
    document.querySelectorAll('.todayBadge').forEach(el => { el.textContent = t.today; });

    const staffTitle = document.querySelector('.staffModalTitle');
    if(staffTitle) staffTitle.textContent = t.saveEmp;
    if(staffInput) staffInput.placeholder = t.saveEmpPh;
    if(staffClear) staffClear.textContent = t.clear;
    if(staffSave) staffSave.textContent = t.save;
    ...
    if(myTrainingTitle) myTrainingTitle.textContent = t.myTrainingTitle;

    document.querySelectorAll('[data-tr-en]').forEach(el => {
      el.textContent = isAr
        ? (el.getAttribute('data-tr-ar') || el.getAttribute('data-tr-en'))
        : el.getAttribute('data-tr-en');
    });

    applyEmployeeNames(isAr);
    ensureNameMap(() => applyEmployeeNames(isAr));
    refreshSaved();
  }
...
def build_top_dock(month_courses: list[dict], in_archive: bool = False) -> str:
    stats = [
    ...
    for value, label in stats:
        faces.append(
            f'<div class="statsFace"><div class="dockValue">{value}</div><div class="dockLabel">{label}</div></div>'
        )
    return f'''
<div class="topDock">
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans+Arabic:wght@400;500;700&family=Plus+Jakarta+Sans:wght@400;500;600;700;800&family=Sora:wght@400;600;700;800;900&display=swap" rel="stylesheet">
  <style>{CSS}</style>
</head>
<body>
<div class="wrap">
  ...
  {build_top_dock(courses, in_archive=in_archive)}

  <div class="myTrainingPanel" id="myTrainingPanel" hidden>
    <div class="myTrainingHead">
      <div>
        <div class="myTrainingTitle" id="myTrainingTitle">My training dates</div>
        <div class="myTrainingSub" id="myTrainingSub"></div>
      </div>
      <div class="myTrainingCountChip dockCard" id="myTrainingCount">
        <div class="dockValue" id="myTrainingCountVal">0</div>
        <div class="dockLabel" id="myTrainingCountLbl">Sessions</div>
      </div>
    </div>
    <div class="myTrainingList" id="myTrainingList"></div>
  </div>
  ...
  <section class="scheduleSection" aria-label="Training schedule">
    ...
    <div class="staffModalCard">
      <div class="staffModalTop">
        <div>
          ...
          <div class="staffModalSub">Uses your roster saved ID if available, or enter staff number / name</div>
        </div>
...
<script defer src="{share_script_src(True)}"></script>
</body>
</html>
'''
