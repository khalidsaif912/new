...
# Next 5 shifts tooltip: desktop hover + mobile long-press on .empRow (injected into page_shell_html).
...
(function initEmployeeNextShiftPreview() {
  ...
  tooltip.innerHTML = '<div class="nextShiftHead"><div id="nextShiftEmp" class="nextShiftEmp">-</div><div class="nextShiftTitle">Upcoming 5 shifts</div></div><div id="nextShiftBody" class="nextShiftBody"></div>';
  ...
  function bindEmployeeRow(rowEl) {
    ...
    rowEl.addEventListener('mouseenter', function(ev) {
      showPreviewForRow(rowEl, ev);
    });
    rowEl.addEventListener('mousemove', moveTooltip);
    rowEl.addEventListener('mouseleave', hideTooltipSoon);
    ...
    rowEl.addEventListener('pointerup', function(ev) {
      if (ev.pointerType !== 'touch') return;
      if (longPressTimer) clearTimeout(longPressTimer);
      longPressTimer = null;
    });
...
#datePicker {{
  ...
  appearance: none;
    ...
    .captureSheet.open {{ display:flex; }}
    .captureSheetCard {{
...
async function captureRosterElement(target, fileNamePrefix) {{
...
(function(){{
  ...
  document.querySelectorAll('.shiftSummary').forEach(function(summary){{
    bindLongPress(summary, function(){{
      ...
      if(!shiftCard) return;
      shiftCard.setAttribute('open', '');
      captureRosterElement(shiftCard, 'shift');
    }});
  }});
}})();
