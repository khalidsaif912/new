...
// ═══════════════════════════════════════════════════
// Department accordion (collapsed by default)
// ═══════════════════════════════════════════════════
(function(){{
  ...
  function collapseAll(exceptCard){{
  ...
  function cardHasSavedEmployee(card){{
    ...
    return rows.some(function(row){{
      ...
      if(!txt) return false;
      if(savedEmpId) {{
        var idRe = new RegExp('(?:^|\\D)' + savedEmpId.replace(/[.*+?^${{}}()|[\\]\\\\]/g, '\\\\$&') + '(?:\\D|$)');
        if(idRe.test(txt)) return true;
      }}
      if(savedEmpName) {{
        return txt.toLowerCase().indexOf(savedEmpName) !== -1;
      }}
      return false;
    }});
  }}
  ...
  deptCards.forEach(function(card){{
    card.classList.add('collapsed');
    var head = card.querySelector('.deptHead');
    if(!head) return;
  ...
  // If a saved employee exists, move their department to top and open it.
  if(savedEmpId || savedEmpName){{
    var targetCard = deptCards.find(cardHasSavedEmployee);
    if(targetCard && targetCard.parentElement){{
      targetCard.parentElement.insertBefore(targetCard, targetCard.parentElement.firstElementChild);
      collapseAll(targetCard);
